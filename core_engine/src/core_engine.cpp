...
double round_to_2_decimal_places(double value) {
...
class Executer{
    ...
    void process_order(event_received er){
        ...
        if(event_type=="place"){
            ...
            if(type=="market"){
                if(mode=="buy") order_price=round_to_2_decimal_places(S1->best_ask*1.05);
                if(mode=="sell") order_price=round_to_2_decimal_places(S1->best_bid*0.95);
